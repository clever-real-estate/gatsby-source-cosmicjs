...
module.exports = async ({
  ...
  if (apiAccess.hasOwnProperty('read_key') && apiAccess.read_key.length !== 0) {
    ...
    apiEndpoint = apiEndpoint + `&limit=${limit}`; // TODO: current bug on hide metafields
  ...
  const documents = await (0, _axios.default)(apiEndpoint, options); // Check for empty object type
  ...
  if (documents.data.total && documents.data.total > limit) {
    ...
    if (isDevelopment) {
      let response = await (0, _axios.default)(apiEndpoint, options);
      ...
      for (let i = 0; i < additionalCallsRequired; i += 1) {
        ...
        let response = await (0, _axios.default)(skipEndpoint, options);
